...
def find_default_csv() -> Optional[Path]:
    # Look for a bofa-emu CSV in ./scripts by default
    cand_dir = Path(__file__).resolve().parent
    matches = sorted(cand_dir.glob("bofa-emu-seat-activity-*.csv"))
    if matches:
...
def main():
    parser = argparse.ArgumentParser(description="Report counts of IDE versions and Copilot extension versions from bofa-emu CSV.")
    parser.add_argument("csv_path", nargs="?", help="Path to CSV (defaults to scripts/bofa-emu-seat-activity-*.csv)")
    ...
    if not csv_path:
        ...
        if not default:
            print("No CSV provided and no default bofa-emu CSV found in scripts/", file=sys.stderr)
            sys.exit(1)
